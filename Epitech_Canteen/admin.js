...
function showAdminDashboard() {
    ...
    renderReviews(); // Also call renderReviews when showing dashboard
}
...
function initSidebarNav() {
    document.querySelectorAll('.sidebar-link').forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (panelId === 'dashboard') { renderRecentOrders(); updateDashboardStats(); renderReviews(); } // Refresh reviews on dashboard
...
function renderAnalytics() {
    ...
    const ordersItem = localStorage.getItem('canteen_orders');
    const orders = ordersItem ? JSON.parse(ordersItem) : [];

    // Category Data
    let plats = 0, boissons = 0, accomp = 0, desserts = 0;

    // Time Data (11h to 16h)
    const hoursCount = { 11: 0, 12: 0, 13: 0, 14: 0, 15: 0, 16: 0 };

    orders.forEach(o => {
        // Categories
        if (o.itemsDetail) {
            o.itemsDetail.forEach(i => {
                // Determine category roughly based on emoji or name since category isn't directly in itemsDetail
                const name = i.name.toLowerCase();
                const emoji = i.emoji;
                if (['🥤', '☕', '🧃'].includes(emoji) || name.includes('jus') || name.includes('eau')) boissons += i.qty;
                else if (['🍰', '🍦', '🍧', '🍩'].includes(emoji) || name.includes('dessert')) desserts += i.qty;
                else if (['🍟', '🥗', '🥖'].includes(emoji) || name.includes('frites') || name.includes('salade')) accomp += i.qty;
                else plats += i.qty;
            });
        }
        ...
        if (o.time) {
            ...
            const timePart = o.time.split(' ')[1];
            if (timePart) {
                const hour = parseInt(timePart.split(':')[0]);
                if (hour >= 11 && hour <= 16) hoursCount[hour]++;
            }
        }
    });
    ...
    if (catCtx) {
        ...
        categoryChartInstance = new Chart(catCtx, {
            type: 'doughnut',
            data: {
                labels: ['Plats', 'Accomp.', 'Boissons', 'Desserts'],
                datasets: [{
                    data: [plats, accomp, boissons, desserts],
    ...
    if (timeCtx) {
        ...
        timeChartInstance = new Chart(timeCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Commandes',
                    data: [hoursCount[11], hoursCount[12], hoursCount[13], hoursCount[14], hoursCount[15], hoursCount[16]],
...
function renderAdminOrders() {
    ...
    list.innerHTML = orders.map(order => {
        ...
        const orderDateDisplay = order.orderDate && order.orderDate !== 'today' ? `<div style="font-size:11px; margin-top:4px; color:var(--warning); font-weight:bold;"><i class="fas fa-calendar-alt"></i> ${order.orderDateText}</div>` : '';
        return `
...
function refreshOrders() {
    if (document.getElementById('admin-app').style.display !== 'none') {
        ...
        renderRecentOrders();
        updateDashboardStats();
        renderReviews(); // Refresh reviews on interval
    }
}
...
function renderRecentOrders() {
    ...
    container.innerHTML = orders.slice(0, 6).map(order => {
        ...
        const orderDateDisplay = order.orderDate && order.orderDate !== 'today' ? `<br><small style="color:var(--warning);font-weight:bold;"><i class="fas fa-calendar-alt"></i> ${order.orderDateText}</small>` : '';
        return `
        <div class="recent-order">
            <span class="ro-name">${order.studentName}${orderDateDisplay}</span>
...
function renderReviews() {
    ...
    const dbOrders = JSON.parse(localStorage.getItem('canteen_orders') || '[]');

    const dashReviews = document.getElementById('dash-recent-reviews');
    if (!dashReviews) return;

    const reviewIds = Object.keys(reviews).sort((a, b) => b - a); // sort by order id DESC

    if (reviewIds.length === 0) {
        dashReviews.innerHTML = `<div style="text-align:center; padding:20px; color:var(--text-muted); grid-column: 1 / -1;">Aucun avis pour le moment.</div>`;
        return;
    }

    dashReviews.innerHTML = reviewIds.slice(0, 6).map(id => {
        const order = dbOrders.find(o => o.id == id);
        if (!order) return '';
        const rating = reviews[id];
        let starHtml = '';
        for (let i = 1; i <= 5; i++) {
            starHtml += `<i class="fas fa-star" style="color:${i <= rating ? 'var(--warning)' : 'var(--glass-border)'}; font-size:12px;"></i>`;
        }

        return `
            <div class="review-card" style="background:var(--bg-lighter); padding:15px; border-radius:var(--radius-sm); border:1px solid var(--glass-border);">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <strong style="color:var(--text-main);">${order.studentName}</strong>
                    <div>${starHtml}</div>
                </div>
                <div style="font-size:12px; color:var(--text-secondary); margin-bottom:5px;">Commande #${id}</div>
                <div style="font-size:13px; color:var(--text-main);">${order.itemsDetail.map(i => `${i.emoji} ${i.name}`).join(', ')}</div>
            </div>
        `;
    }).join('');
}
